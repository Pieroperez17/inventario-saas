  ...
  return (
    <div className="tarjeta flex items-center gap-4">
      ...
      <div className="min-w-0">
        <p className="truncate text-sm text-slate-400">{etiqueta}</p>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Metrica icono={Package} etiqueta="Productos" valor={formatearNumero(tarjetas.totalProductos)} color="bg-marca-500/20 text-marca-300" />
        <Metrica icono={DollarSign} etiqueta="Valor del inventario" valor={formatearMoneda(tarjetas.valorInventario, moneda)} color="bg-emerald-500/20 text-emerald-300" />
        <Metrica icono={Boxes} etiqueta="Stock total (unidades)" valor={formatearNumero(tarjetas.stockTotal)} color="bg-sky-500/20 text-sky-300" />
        <Metrica icono={AlertTriangle} etiqueta="Productos bajo mínimo" valor={formatearNumero(tarjetas.productosBajoMinimo)} color="bg-amber-500/20 text-amber-300" />
      </div>

      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Metrica icono={Warehouse} etiqueta="Almacenes" valor={tarjetas.totalAlmacenes} color="bg-slate-500/20 text-slate-300" />
        <Metrica icono={Store} etiqueta="Tiendas" valor={tarjetas.totalTiendas} color="bg-slate-500/20 text-slate-300" />
        <Metrica icono={Users} etiqueta="Usuarios" valor={tarjetas.totalUsuarios} color="bg-slate-500/20 text-slate-300" />
      </div>

      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="tarjeta">
          <h3 className="mb-4 font-semibold text-slate-100">Existencias por ubicación</h3>
        ...
        <div className="tarjeta">
          <h3 className="mb-4 font-semibold text-slate-100">Top productos por stock</h3>
          {topProductos.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={topProductos} layout="vertical" margin={{ left: 20 }}>
                ...
                <YAxis type="category" dataKey="sku" width={70} tick={{ fill: '#94a3b8', fontSize: 12 }} />
                <Tooltip contentStyle={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 8, color: '#f1f5f9' }} />
      ...
      <div className="mt-6 tarjeta">
        <h3 className="mb-4 font-semibold text-slate-100">Movimientos recientes</h3>
            ...
            {movimientosRecientes.map((m) => (
              <li key={m.id} className="flex items-center justify-between gap-3 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-slate-100">{m.producto.nombre}</p>
                    <p className="text-xs text-slate-500">{m.producto.sku}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold text-slate-200">{formatearNumero(m.cantidad)}</p>
